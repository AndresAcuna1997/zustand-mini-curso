...
export const JiraTasks = ( { title, tasks, status }: Props ) => {
  // const changeStatusTask = useTaskStore((state) => state.changeStatusTask);
  const { handleDragOver, handleDragLeave, handleOnDrop, isDragging, onDragOver, handleAddTask } = useTasks( status );
    ...
    <div
      ...
      className={ classNames(
        "!text-black border-4  relative flex flex-col rounded-[20px]  bg-white bg-clip-border shadow-3xl shadow-shadow-500  w-full !p-4 3xl:p-![18px]",
        {
          ...
          "border-green-500 border-dashed": onDragOver && isDragging,
        }
      ) }
    >
      {/* Task Header */ }
